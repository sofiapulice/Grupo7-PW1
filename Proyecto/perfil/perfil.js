...
let regexCampoPassword =
  /^(?=.*[a-zA-Z].*[a-zA-Z])(?=.*\d.*\d)(?=.*[!@#$%^&*(),.?":{}|<>[\]\\/+=_-].*[!@#$%^&*(),.?":{}|<>[\]\\/+=_-])[a-zA-Z0-9!@#$%^&*(),.?":{}|<>[\]\\/+=_-]{8,}$/;
...
function validarPerfil(event) {
  ...
  const metodosDePago = document.querySelector(
  ...
  if (!metodosDePago) {
    ...
    if (tarjetaPar === "") {
      ...
      const sumaDeTodoMenosElUltimo = digitos
        .slice(0, -1)
...
document
  .querySelector(".button-confirmar")
  .addEventListener("click", validarPerfil);

document
  .querySelector(".contenedor__form")
